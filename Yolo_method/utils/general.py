...
def check_file(file, suffix=""):
    ...
    if os.path.isfile(file) or not file:  # exists
        ...
        files = []
        # First try to find file relative to ROOT directory
        root_file = ROOT / file
        if root_file.exists():
...
def check_font(font=FONT, progress=False):
    ...
    if not font.exists() and not file.exists():
        try:
            ...
            LOGGER.warning(f"Font download failed: {e}. Using system default font.")
            # Create a dummy font file to avoid repeated download attempts
